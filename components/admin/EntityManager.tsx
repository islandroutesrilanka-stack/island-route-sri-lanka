...
import {
  Plus, Pencil, Trash2, X, Loader2, Eye, EyeOff, AlertTriangle,
} from "lucide-react";
...
export default function EntityManager({ config }: { config: EntityConfig }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(e.message?.startsWith('"') ? e.message : `Invalid JSON: ${e.message}`);
      return;
    }
    setSaving(true);
    const res =
      editing === "new"
        ...
        : await sb.from(config.table).update(payload).eq("id", (editing as Row).id);
    setSaving(false);
    if (res.error) {
      ...
      <div className="mt-8 border border-ink/10 bg-white/70 divide-y divide-ink/5">
        {loading ? (
          ...
          <p className="p-6 text-sm text-ink/50">
            ...
            content until you add rows (or run supabase/seed.sql to import it).
          </p>
        ) : (
          rows.map((row) => (
            <div
          ...
          >
            ...
            >
              <div className="sticky top-0 z-10 flex items-center justify-between border-b border-ink/10 bg-sand px-6 py-4">
                ...
                <button onClick={() => setEditing(null)} className="p-2 text-ink/50 hover:text-ink">
                  <X size={20} />
                </button>
              </div>
